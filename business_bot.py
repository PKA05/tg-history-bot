...
def init_db():
    ...
    # ПРОВЕРКА: Если таблица старая (без колонки time_str), мы её обновим
    try:
        cursor.execute("SELECT time_str FROM messages LIMIT 1")
    except sqlite3.OperationalError:
        # Если колонки нет, удаляем старую таблицу, чтобы пересоздать её заново
        cursor.execute("DROP TABLE IF EXISTS messages")
        print("🔄 [DB] Старая таблица удалена для обновления структуры.")
...
def save_to_db(msg_id, content_type, text=None, file_id=None, sender_name="Неизвестно", chat_title="Личный чат"):
    try:
        ...
        conn.commit()
        conn.close()
        print(f"💾 [DB] Сохранено ({content_type}) {msg_id} от {sender_name} | Время: {time_str} | Дата: {today_date}")
...
def send_welcome(message):
    ...
    status_text = (
        "🟢 Бизнес-бот Архивариус V3.4 (UTC+5 Ташкент) активен!\n\n"
...
def handle_date_selection(call):
    ...
    for idx, msg in enumerate(messages, 1):
        ...
        # Красивое отображение с временем отправки по Ташкенту
        report += (
            f"{idx}. 🕒 Время: {msg_time} | ID: {msg_id}\n"
            f"👤 От: {sender}\n"
            f"💬 Чат: {chat}\n"
