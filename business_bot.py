...
# ==========================================
# ТОЧНОЕ ОПРЕДЕЛЕНИЕ ДАТЫ ПО ТАШКЕНТУ (UTC+5)
# ==========================================
def get_tashkent_date(days_offset=0):
    """
    Возвращает дату в формате YYYY-MM-DD для часового пояса Ташкента (UTC+5)
    с возможностью сдвига на days_offset дней назад/вперед.
    """
    # Получаем текущее время по UTC, добавляем 5 часов (Ташкент)
    tashkent_now = datetime.utcnow() + timedelta(hours=5)
    # Применяем смещение в днях, если нужно (например, -1 для "Вчера")
...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS messages (
            ...
            sender_name TEXT,
            chat_title TEXT,
            date_str TEXT,     -- Дата отправки (YYYY-MM-DD) по Ташкенту
            timestamp REAL
        )
    ''')
    conn.commit()
...
def save_to_db(msg_id, content_type, text=None, file_id=None, sender_name="Неизвестно", chat_title="Личный чат"):
    try:
        ...
        # Получаем дату строго по Ташкенту
        today_date = get_tashkent_date(days_offset=0)
        now_ts = time.time()
        ...
        conn.commit()
        conn.close()
        print(f"💾 [DB] Сохранено ({content_type}) {msg_id} от {sender_name} | Дата записи: {today_date}")
