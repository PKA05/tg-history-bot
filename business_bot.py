...
# ==========================================
# ПОЛУЧЕНИЕ ВРЕМЕНИ ПО UTC+5 (Ташкент)
# ==========================================
def get_local_date_str(offset_hours=0):
    """Возвращает дату в формате YYYY-MM-DD с учетом сдвига (в днях) и часового пояса UTC+5"""
    # Сначала получаем строгое UTC-время, затем добавляем 5 часов (Ташкент) и вычитаем/добавляем дни (offset_hours)
    utc_now = datetime.utcnow()
    local_now = utc_now + timedelta(hours=5) + timedelta(hours=offset_hours)
...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS messages (
            ...
            sender_name TEXT,
            chat_title TEXT,
            date_str TEXT,     -- Дата отправки (YYYY-MM-DD) по UTC+5
            timestamp REAL
        )
    ''')
    conn.commit()
...
def save_to_db(msg_id, content_type, text=None, file_id=None, sender_name="Неизвестно", chat_title="Личный чат"):
    try:
        ...
        # Получаем дату по Ташкенту (UTC+5)
        ...
        conn.commit()
        conn.close()
        print(f"💾 [DB] Успешно сохранено ({content_type}) {msg_id} от {sender_name} | Дата: {today_date}")
...
# ==========================================
# 1. ОБЫЧНЫЕ КОМАНДЫ И ПОИСК ПО ДАТАМ
# ==========================================
...
def send_welcome(message):
    ...
    status_text = (
        "🟢 Бизнес-бот Архивариус V3 (UTC+5 Ташкент) активен!\n\n"
...
def show_history_menu(message):
    ...
    # Считаем даты по Ташкенту
    today = get_local_date_str(offset_hours=0)
    yesterday = get_local_date_str(offset_hours=-24)
    prev_day = get_local_date_str(offset_hours=-48)
